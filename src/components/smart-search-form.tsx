  ...
  return (
    <Card className="w-full max-w-2xl">
      <CardHeader>
        ...
        <CardDescription>
          Ask a question about your business, and we&apos;ll suggest relevant reports or data views.
          For example: &quot;What were my sales last quarter?&quot;
        </CardDescription>
      </CardHeader>
          ...
          <div className="space-y-1">
            ...
            {state?.errors?.query && (
              <p id="query-error" className="text-sm font-medium text-destructive flex items-center gap-1">
                <AlertTriangle className="h-4 w-4" /> {state.errors.query.join(', ')}
              </p>
            )}
          </div>
          <SubmitButton />
      ...
      {state?.message && !state.errors?.query && (state.suggestions && state.suggestions.length > 0 || state.message === 'No specific suggestions found. Try rephrasing your query.') && (
